...
static void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain) {
    ...
    if (context->device.graphics_queue_index != context->device.present_queue_index) {
        ...
        swapchain_create_info.pQueueFamilyIndices = &context->device.graphics_queue_index;
    }
    ...
    if (!swapchain->images) {
        swapchain->images = kallocate(sizeof(VkImage) * image_count, MEMORY_TAG_RENDERER);
    }
    if (!swapchain->views) {
        swapchain->views = kallocate(sizeof(VkImageView) * image_count, MEMORY_TAG_RENDERER);
    }
